...
categories: [jekyll, quarto, website, github-action, netlify, deployment, 2023]
...
[Quarto's publishing guide](https://quarto.org/docs/publishing/netlify.html#github-action) is extremely helpful to prep and set up the Netlify + Github Actions workflow, but I need to consider the Netlify `_redirects` file.

Instead of rendering `_redirects` every time like [Danielle Navarro does](https://blog.djnavarro.net/posts/2022-04-20_porting-to-quarto/#fnref3), I did a trick by uploading the file to root folder, and using the Github Actions to move it to `_site` folder after it completes the render. Here is my workflow file:
...
2. No need to render `_redirects` every time, thereby avoiding the need to deal with installing R/Python/dependencies in Github Actions, which is required if you follow Danielle Navarro's method.
