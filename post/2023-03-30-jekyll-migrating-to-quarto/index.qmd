...
The good news is, I don't need to update the `_redirects` every time, the data is the same since it just converts old post links to the new ones. So I did a trick by uploading the `_redirects` file to root folder, and using the Github Actions to move it to `_site` folder **after** it completes the render. Here is my workflow file:
